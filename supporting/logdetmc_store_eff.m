function [ld,iter,Zinv12eps] = logdetmc_store_eff(A,G, eps, ldG)

%Compute logdet(G)
%ldG = logdetstable(G,true);

%Estimate the trace of log(GAG')
tol = 1.e-7; %tol = 1.e-5;     %tol = 1.e-7;   
maxiter = 350;
n = size(G,1);
ns = size(eps,2);
...
parfor i = 1:ns
    % for i = 1:ns
    [tr,relres,Vklst,Tklst,nrmblst] = bilinearform(A,G,eps(:,i),@(x)log(x),maxiter,tol);
    mc = mc + tr;    %eps(:,i)'*xl;
    ...
    e1 = zeros(k,1);   
    e1(1) = nrmblst;
    Zinv12eps(:,i) = Vklst*(sqrtm(Tklst)\e1);
end

fileid=fopen('iterations.txt','a+'); fprintf(fileid,'%5d\n',iter); fclose(fileid);
mc = n*mc/ns;
