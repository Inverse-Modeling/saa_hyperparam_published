function [F, gradF, val] = objfun_mc_symm(theta, inv)
%
...
elseif length(theta) ==  4
  [~, gradP, logP] = Prior4(inv.prior_type);
    ...
    eps = inv.eps;
    ns  = size(eps,2);
    [ld,lan_iter,lanc] = logdetmc_store(Zmat,G,eps,ldG);

    % Implements: Z^{-1}(A*mu-d)
    % [Zinvres, relres] = krylov_solve(Zmat, G, res, 100, 1.e-6);
    [Zinvres, relres] = krylov_solve(Zmat, G, res, 300, 1.e-8);
    ...
    %for j = 1:ns
        ...
        nrmb = lanc.nrmblst{j};
        k = size(Tk,1);
        e1 = zeros(k,1);   e1(1) = nrmb;
        Zinv12eps(:,j) = Vk*(sqrtm(Tk)\e1);
     end
    ...
    Gteps = G'*Zinv12eps;
    for i = 1:hyp_dim 
    	  Z_der_eps = A*(dQ{i,1}*(A'*Gteps)) + dR{i,1}*Gteps;
        alpha_i = trace(Z_der_eps'*Gteps)/(2*ns);
