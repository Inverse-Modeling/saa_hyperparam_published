    ...
    function y = mtimes(arg1, arg2) % Overload matrix vector multiplicaiton
      ...
      if isa(arg1,'kronMat')
        ...
        if isa(arg2,'double') && length(arg2) == 1
          ...
          % end
          if numel(arg2) == length(arg2) % arg2 is a vector
          ...
          elseif size(arg1.B,2)*size(arg1.C,2) == length(arg2) | size(arg1.B,1)*size(arg1.C,1) == length(arg2) % arg2 is a matrix of correct size

            if iscell(arg1.B)
              error('kronMat/mtimes multiplication with cell not implemented')
            else % isa matrix
              [m1, m2] = size(arg1.C);
              [n1, n2] = size(arg1.B);
              ...
              if arg1.transpose
                % Matrix transpose times vector
                for i = 1:size(arg2,2)
                  y_temp = arg1.C'*reshape(arg2(:,i),m1,n1)*arg1.B;
                  y(:,i) = y_temp(:);
                end
              else
                % Matrix times vector
                for i = 1:size(arg2,2)
                  y_temp =  arg1.C*reshape(arg2(:,i),m2,n2)*arg1.B';
                  y(:,i) = y_temp(:);
                end
              end
            end
          else
            error('kronMat/mtimes assumes input is a vector or matrix')
          end
        end
